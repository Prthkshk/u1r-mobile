...
export default function OrderSummary({ navigation }) {
  ...
  const loadOrder = useCallback(async () => {
    ...
    try {
      ...
      console.log("Order details load error:", err?.response?.data || err?.message);
    } finally {
      setLoading(false);
    }
  }, [orderId]);
    ...
    if (normalizedStatus === "DISPATCHED") {
      return {
        ...
        sub: order.expectedDelivery
          ...
          : `Dispatched on ${formatDate(order.dispatchedAt || order.updatedAt || order.createdAt)}`,
        color: "#1E88E5",
        icon: "DISPATCHED",
      };
    }
  ...
  const handleCancel = async () => {
    if (!order?._id || normalizeOrderStatus(order?.status) !== "PENDING") return;
    setConfirmCancelOpen(true);
  };

  const confirmCancel = async () => {
    if (!order?._id || normalizeOrderStatus(order?.status) !== "PENDING") {
  ...
  const getImageSource = (item) => {
    ...
    if (!raw || raw.toLowerCase().includes("placeholder")) return placeholder;
    return { uri: withBaseUrl(raw) };
  };
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 30 }}>
          <View style={styles.summaryCard}>
            <View style={styles.summaryHeader}>
              {normalizeOrderStatus(order?.status) === "DISPATCHED" ? (
...
const styles = StyleSheet.create({
  ...
  downloadRow: { flexDirection: "row", alignItems: "center", gap: 10, marginTop: 2 },
  downloadIcon: { width: 18, height: 18, tintColor: "#1AA145", resizeMode: "contain" },
